...
cid number(2) constraint c_primkey primary key,
lname char(20),
fname char(20)
);
...
insert into customers values(21,'APPIAH','NITIN');


REM PRODUCTS (product number,  Flavor, Food, Price)
create table products(
pid varchar2(15) constraint p_primkey primary key,
flavor char(15),
food char(15),
price float(10)
);
...
rno number(5) constraint r_primkey primary key,
rdate date,
cid number(2) constraint r_cid_ref references customers(cid)
);
REM --------------------------------------------------------------------------
...
rno number(5) constraint i_rno_ref references Receipts(rno),
ordinal number(1),
item varchar2(15) constraint i_item_ref references products(pid),
constraint i_primkey primary key(rno, ordinal)
);
REM --------------------------------------------------------------------------
...
group by item having count(*) >= all
...
select count(count(*)) from item_list 
where item in 
(select pid from products p1 where price > 
(select avg(price) from products p2 
group by food having p1.food = p2.food)) 
group by rno;
...
where i.rno in
(select i.rno from item_list i join products p on(i.item = p.pid) 
...
where i.item in( select p.pid from products p join item_list i on(p.pid = i.item)
group by p.pid 
having count(*) <= all
(select count(*) from products p join item_list i on(p.pid = i.item) 
group by p.pid));
...
(select count(distinct flavor) from products 
where food = 'Meringue'));
...
(select flavor from products where food='Tart')) and food not in('Meringue', 'Tart');
